...
def generate_p_raw_from_scaffold(features, context, phase, yes_bid=50.0, no_bid=50.0):
    ...
    if not decision.tradable or not decision.side:
        raise SignalSourceError(f"Scorer returned no tradable signal: {decision.reason}")
...
def get_live_market_snapshot():
    try:
        ...
        return {"market_ticker": market.ticker, "strike": float(market.ticker.split("-T")[-1]),
...
class Collector:
    ...
    def run(self):
        ...
        cycle_idx = 0; filled = skipped = errors = 0; start_time = time.time()
        signal_sources: dict = {}
    ...
    def _get_cycle_data(self, cycle_idx):
        ...
        if self.mode == "loop-smoke":
            ...
            snap = dict(self.snapshots[cycle_idx % len(self.snapshots)])
            snap["expiry"] = base_expiry
